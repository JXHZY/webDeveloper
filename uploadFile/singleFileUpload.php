...
{
	//$maxSize 字节大小 默认2M
	//$allowExt 允许上传文件的类型(大小写是有区别的)
	//$path 上传到哪个目录下 默认为'uploads'
	//$flag 是否要求检测上传的图片为真实的图片类型，真=>检测, 默认为真->检测
	...
	if($fileInfo['error']>0)
	{
		...
		{
			case 1:
				$mes = 'Oversize of upload_max_filesize in PHP.ini';			//上传文件超过了PHP配置文件中upload_max_filesize选项的值
				break;
			case 2:
				$mes = 'Over size of MAX_FILE_SIZE!';      //超过了表单MAX_FILE_SIZE限制的大小
				break;
			case 3:
				$mes = 'Just upload some parts of the file!';		//文件部分被上传
				break;
			case 4:
				$mes = 'Didn\'t choose any file to upload!';		//没有选择上传文件
				break;
			case 6:
				$mes = 'Didn\'t find the temporary directory!';		//没有找到临时目录
				break;
			case 7:
			case 8:
				$mes = 'System Error!';				//系统错误
				break;
		}
		exit($mes);
	}
